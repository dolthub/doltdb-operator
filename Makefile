...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name dolt-operator-builder
	$(CONTAINER_TOOL) buildx use dolt-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm dolt-operator-builder
	rm Dockerfile.cross
...
.PHONY: install
install: kubectl manifests kustomize ## Install CRDs into the K8s cluster specified in ~/.kube/config.
